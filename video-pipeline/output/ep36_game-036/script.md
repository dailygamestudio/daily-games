...
**Episode**: #36 — Neon Ricochet - Day 36  
...
|-----------|---------------------|----------------------|--------------------------------|
| **0:00-0:08** | **[COLD OPEN]**<br>• Split screen: Broken game (red console errors) → Fixed game (smooth 60fps)<br>• Overlay: "AI built this in 24 hours. Then it broke itself." | *(No voiceover — ambient synth music, glitch SFX on broken side)* | **Lower-third**: `Daily Game Studio Devlog #36`<br>`Neon Ricochet - Day 36` |
| **0:08-0:15** | **[TITLE SEQUENCE]**<br>• Fast montage: Code generating → Playwright test running → NIM API fixing → Git push → Live on GitHub Pages<br>• Series logo animation | "Every day, an AI agent builds a complete HTML5 game from scratch. But the real story isn't the code it writes — it's the code it *fixes*." | **Animated title card**: `Daily Game Studio Devlog`<br>Episode `36: Neon Ricochet - Day 36` |
| **0:15-0:35** | **[ACT 1: THE CHALLENGE]**<br>• Screen-recorded gameplay of Neon Ricochet - Day 36 (clean run)<br>• Diagram: Game genre classification + core mechanic<br>• Code snippet: Main game loop / entity system | "Day 36. The assignment: Ricochet shooting with angle prediction, limited shots, targets. Trajectory prediction line, wall bounce physics, shot counter, target destruction chain reaction.<br><br>No frameworks. No libraries. Vanilla HTML5 Canvas, 400 lines max. The AI has one shot to get the physics, input, and rendering right." | **Code callout**: `gameLoop()` / `update()` / `draw()`<br>**Diagram**: `ricochet_mechanic.excalidraw` |
| **0:35-1:00** | • Archive: First commit diff (green lines only)<br>• index.json entry highlight<br>• GitHub Pages URL typing | "First commit pushed at 2026-08-20 00:00. Live at `dailygamestudio.github.io/daily-games/games/game-036/`<br><br>But the self-healing pipeline hadn't run yet. And that's where things get interesting." | **Overlay**: `git log --oneline -1`<br>`unknown Initial commit` |
| **1:00-1:30** | **[ACT 2: THE STRUGGLE]**<br>• Playwright test recording: Red failures, console errors<br>• Bug cards sliding in: `CRITICAL: JS_ERROR`, `MAJOR: MISSING_START_BUTTON`<br>• Terminal: `node test-runner.js --priority-only` | "The self-healing loop kicks in. Playwright loads the live page, simulates keystrokes, checks for game-over state.<br><br>And it finds: 2 bugs. 1 critical. The game *looks* done. It's not." | **Bug cards** (animated):<br>🔴 `JS_ERROR` — `JavaScript errors found: Unexpected end of input`<br>🟠 `MISSING_START_BUTTON` — `No start button found`<br>... |
...
| **2:00-2:30** | • Retry loop: Attempt 1 → still failing → Attempt 2 → PASS<br>• Test report: Green checkmarks | "First fix attempt: Fixed primary issue, introduced minor regression.<br>Second attempt: PASS — all tests green.<br><br>The auto-fixer has a 3-retry limit with 30-second backoff. This game took 2 attempts and 4 minutes." | **Terminal log**:<br>`✅ game-036 fixed and verified!`<br>`🔄 Cycle 1 complete` |
...
| **3:30-4:00** | **[ACT 4: PLAYTEST & REFLECTION]**<br>• Full gameplay recording (no cuts)<br>• HUD stats: Score, Level, FPS counter<br>• Mobile touch demo (if applicable) | "Let's play the final version. Play the polished ricochet with all bugs fixed. Smooth 60fps, responsive controls, persistent high scores..<br><br>Notice: particle systems, ricochet-specific mechanics, and localStorage persistence — that's requestAnimationFrame with fixed timestep running at 60fps on a single Canvas context." | **Live HUD overlay**: `FPS: 60` `Entities: ~35 (entities + particles + UI)` |
| **4:00-4:20** | • Stats card: Lines of code, Dev time, Fix cycles, Final bug count<br>• Next episode teaser: Next game thumbnail + title | "Neon Ricochet - Day 36: 350 lines, 1.8 hours, 1 fix cycles, 0 bugs shipped.<br><br>Tomorrow: **Neon Gravity Runner - Day 37**. Variable gravity platformer with gravity flipping, momentum." | **End card**: `Daily Game Studio Devlog #36`<br>`Next: #37 Neon Gravity Runner - Day 37`<br>`Subscribe for daily AI gamedev` |
...
**Title**: `Daily Game Studio Devlog #36: Neon Ricochet - Day 36 — AI Builds & Fixes a Game in 24h`
...
🎮 PLAY Neon Ricochet - Day 36: https://dailygamestudio.github.io/daily-games/games/game-036/
📂 SOURCE CODE: https://github.com/dailygamestudio/daily-games/tree/main/games/game-036
🤖 HERMES AGENT: https://hermes-agent.nousresearch.com/

CHAPTERS:
0:00 Cold Open — Broken vs Fixed
0:15 The Challenge: Ricochet shooting with angle prediction, limited shots, targets
...
**Thumbnail Text**: `AI Built This Game` + `Then Fixed Its Own Bugs` + `#36`
