...
**Episode**: #37 — Neon Gravity Runner - Day 37  
...
|-----------|---------------------|----------------------|--------------------------------|
| **0:00-0:08** | **[COLD OPEN]**<br>• Split screen: Broken game (red console errors) → Fixed game (smooth 60fps)<br>• Overlay: "AI built this in 24 hours. Then it broke itself." | *(No voiceover — ambient synth music, glitch SFX on broken side)* | **Lower-third**: `Daily Game Studio Devlog #37`<br>`Neon Gravity Runner - Day 37` |
| **0:08-0:15** | **[TITLE SEQUENCE]**<br>• Fast montage: Code generating → Playwright test running → NIM API fixing → Git push → Live on GitHub Pages<br>• Series logo animation | "Every day, an AI agent builds a complete HTML5 game from scratch. But the real story isn't the code it writes — it's the code it *fixes*." | **Animated title card**: `Daily Game Studio Devlog`<br>Episode `37: Neon Gravity Runner - Day 37` |
| **0:15-0:35** | **[ACT 1: THE CHALLENGE]**<br>• Screen-recorded gameplay of Neon Gravity Runner - Day 37 (clean run)<br>• Diagram: Game genre classification + core mechanic<br>• Code snippet: Main game loop / entity system | "Day 37. The assignment: Variable gravity platformer with gravity flipping, momentum. Global gravity vector flip, momentum conservation across flip, platform attachment, checkpoint.<br><br>No frameworks. No libraries. Vanilla HTML5 Canvas, 400 lines max. The AI has one shot to get the physics, input, and rendering right." | **Code callout**: `gameLoop()` / `update()` / `draw()`<br>**Diagram**: `gravity runner_mechanic.excalidraw` |
| **0:35-1:00** | • Archive: First commit diff (green lines only)<br>• index.json entry highlight<br>• GitHub Pages URL typing | "First commit pushed at 2026-08-20 00:00. Live at `dailygamestudio.github.io/daily-games/games/game-037/`<br><br>But the self-healing pipeline hadn't run yet. And that's where things get interesting." | **Overlay**: `git log --oneline -1`<br>`unknown Initial commit` |
| **1:00-1:30** | **[ACT 2: THE STRUGGLE]**<br>• Playwright test recording: Red failures, console errors<br>• Bug cards sliding in: `CRITICAL: JS_ERROR`, `MAJOR: MISSING_START_BUTTON`<br>• Terminal: `node test-runner.js --priority-only` | "The self-healing loop kicks in. Playwright loads the live page, simulates keystrokes, checks for game-over state.<br><br>And it finds: 1 bugs. 0 critical. The game *looks* done. It's not." | **Bug cards** (animated):<br>🔴 `GAME_NOT_STARTED` — `Start button clicked but overlay still visible`<br>🟠 `NONE` — ``<br>... |
| **1:30-2:00** | • Split view: Buggy code (left) → NIM API prompt (center) → Fixed code (right)<br>• Highlight: Specific line causing crash<br>• Auto-fixer.js execution log | "Each bug gets packaged into a structured prompt for NIM API — Nemotron 3 Ultra. The prompt includes: the full HTML, the bug list with severity, and strict requirements: fix everything, keep the neon aesthetic, add pause/menu overlays, support touch.<br><br>Let's look at the worst offender: **GAME_NOT_STARTED: Start button clicked but overlay still visible**." | **Code diff** (syntax highlighted):<br>```diff<br>- overlay.classList.remove('hidden')  // Never hides<br>+ overlay.classList.add('hidden'); running = true; requestAnimationFrame(loop);<br>``` |
| **2:00-2:30** | • Retry loop: Attempt 1 → still failing → Attempt 2 → PASS<br>• Test report: Green checkmarks | "First fix attempt: Fixed primary issue, introduced minor regression.<br>Second attempt: PASS — all tests green.<br><br>The auto-fixer has a 3-retry limit with 30-second backoff. This game took 1 attempts and 2 minutes." | **Terminal log**:<br>`✅ game-037 fixed and verified!`<br>`🔄 Cycle 1 complete` |
| **2:30-3:00** | **[ACT 3: THE BREAKTHROUGH]**<br>• Side-by-side: Before fix / After fix gameplay<br>• Code walkthrough: The architectural fix (not just patch)<br>• Diagram: Why this fix solves the root cause | "Here's what actually broke: Overlay hide logic tied to specific variable name ('running') but game used 'gameState'.<br><br>The fix wasn't just 'add missing button' — it was **State machine unification: single source of truth for game state, all UI derives from it**.<br><br>This is the pattern across all 54 games: the AI learns to build *testable* architecture, not just working code." | **Architecture diagram**: `{{ARCH_DIAGRAM}}`<br>**Key code block**: `gameState enum + derived UI visibility (start/playing/paused/gameover)` |
| **3:00-3:30** | • Version bump in index.json: `"version": 1` → `"version": 2`<br>• Git commit: "Auto-fix: Self-healing bug fixes"<br>• GitHub Actions deploy log | "Version bumped. Committed. Pushed. GitHub Pages rebuilds in ~2 minutes.<br><br>Now the live version is the *fixed* version. Players never see the broken one." | **Overlay**: `git diff index.json`<br>`+ "version": 2`<br>`+ "last_updated": "2026-08-20"` |
| **3:30-4:00** | **[ACT 4: PLAYTEST & REFLECTION]**<br>• Full gameplay recording (no cuts)<br>• HUD stats: Score, Level, FPS counter<br>• Mobile touch demo (if applicable) | "Let's play the final version. Play the polished gravity runner with all bugs fixed. Smooth 60fps, responsive controls, persistent high scores..<br><br>Notice: particle systems, gravity runner-specific mechanics, and localStorage persistence — that's requestAnimationFrame with fixed timestep running at 60fps on a single Canvas context." | **Live HUD overlay**: `FPS: 60` `Entities: ~32 (entities + particles + UI)` |
| **4:00-4:20** | • Stats card: Lines of code, Dev time, Fix cycles, Final bug count<br>• Next episode teaser: Next game thumbnail + title | "Neon Gravity Runner - Day 37: 329 lines, 1.6 hours, 1 fix cycles, 0 bugs shipped.<br><br>Tomorrow: **Neon Lights Out - Day 38**. Lights Out puzzle with toggle propagation, minimal moves." | **End card**: `Daily Game Studio Devlog #37`<br>`Next: #38 Neon Lights Out - Day 38`<br>`Subscribe for daily AI gamedev` |
...
**Title**: `Daily Game Studio Devlog #37: Neon Gravity Runner - Day 37 — AI Builds & Fixes a Game in 24h`
...
🎮 PLAY Neon Gravity Runner - Day 37: https://dailygamestudio.github.io/daily-games/games/game-037/
📂 SOURCE CODE: https://github.com/dailygamestudio/daily-games/tree/main/games/game-037
🤖 HERMES AGENT: https://hermes-agent.nousresearch.com/

CHAPTERS:
0:00 Cold Open — Broken vs Fixed
0:15 The Challenge: Variable gravity platformer with gravity flipping, momentum
1:00 The Struggle: 1 Bugs Found
2:30 The Breakthrough: State machine unification: single source of truth for game state, all UI derives from it
...
**Thumbnail Text**: `AI Built This Game` + `Then Fixed Its Own Bugs` + `#37`
