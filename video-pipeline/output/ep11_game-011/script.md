...
**Episode**: #11 — Neon Asteroids - Day 11  
...
|-----------|---------------------|----------------------|--------------------------------|
| **0:00-0:08** | **[COLD OPEN]**<br>• Split screen: Broken game (red console errors) → Fixed game (smooth 60fps)<br>• Overlay: "AI built this in 24 hours. Then it broke itself." | *(No voiceover — ambient synth music, glitch SFX on broken side)* | **Lower-third**: `Daily Game Studio Devlog #11`<br>`Neon Asteroids - Day 11` |
| **0:08-0:15** | **[TITLE SEQUENCE]**<br>• Fast montage: Code generating → Playwright test running → NIM API fixing → Git push → Live on GitHub Pages<br>• Series logo animation | "Every day, an AI agent builds a complete HTML5 game from scratch. But the real story isn't the code it writes — it's the code it *fixes*." | **Animated title card**: `Daily Game Studio Devlog`<br>Episode `11: Neon Asteroids - Day 11` |
| **0:15-0:35** | **[ACT 1: THE CHALLENGE]**<br>• Screen-recorded gameplay of Neon Asteroids - Day 11 (clean run)<br>• Diagram: Game genre classification + core mechanic<br>• Code snippet: Main game loop / entity system | "Day 11. The assignment: Asteroids clone with ship inertia, screen wrap, splitting asteroids. Ship inertia physics, screen wrap, asteroid splitting (large→medium→small), UFO spawning, hyperspace.<br><br>No frameworks. No libraries. Vanilla HTML5 Canvas, 400 lines max. The AI has one shot to get the physics, input, and rendering right." | **Code callout**: `gameLoop()` / `update()` / `draw()`<br>**Diagram**: `asteroids_mechanic.excalidraw` |
| **0:35-1:00** | • Archive: First commit diff (green lines only)<br>• index.json entry highlight<br>• GitHub Pages URL typing | "First commit pushed at 2026-08-18 00:00. Live at `dailygamestudio.github.io/daily-games/games/game-011/`<br><br>But the self-healing pipeline hadn't run yet. And that's where things get interesting." | **Overlay**: `git log --oneline -1`<br>`unknown Initial commit` |
| **1:00-1:30** | **[ACT 2: THE STRUGGLE]**<br>• Playwright test recording: Red failures, console errors<br>• Bug cards sliding in: `CRITICAL: JS_ERROR`, `MAJOR: MISSING_START_BUTTON`<br>• Terminal: `node test-runner.js --priority-only` | "The self-healing loop kicks in. Playwright loads the live page, simulates keystrokes, checks for game-over state.<br><br>And it finds: 1 bugs. 1 critical. The game *looks* done. It's not." | **Bug cards** (animated):<br>🔴 `JS_ERROR` — `JavaScript errors found: Identifier 'angle' has already been declared`<br>🟠 `NONE` — ``<br>... |
| **1:30-2:00** | • Split view: Buggy code (left) → NIM API prompt (center) → Fixed code (right)<br>• Highlight: Specific line causing crash<br>• Auto-fixer.js execution log | "Each bug gets packaged into a structured prompt for NIM API — Nemotron 3 Ultra. The prompt includes: the full HTML, the bug list with severity, and strict requirements: fix everything, keep the neon aesthetic, add pause/menu overlays, support touch.<br><br>Let's look at the worst offender: **JS_ERROR: JavaScript errors found: Identifier 'angle' has already been**." | **Code diff** (syntax highlighted):<br>```diff<br>- // Runtime error in game loop<br>+ try { update(); draw(); } catch(e) { console.error(e); recovery(); }<br>``` |
| **2:00-2:30** | • Retry loop: Attempt 1 → still failing → Attempt 2 → PASS<br>• Test report: Green checkmarks | "First fix attempt: Fixed primary issue, introduced minor regression.<br>Second attempt: PASS — all tests green.<br><br>The auto-fixer has a 3-retry limit with 30-second backoff. This game took 1 attempts and 2 minutes." | **Terminal log**:<br>`✅ game-011 fixed and verified!`<br>`🔄 Cycle 1 complete` |
...
| **3:30-4:00** | **[ACT 4: PLAYTEST & REFLECTION]**<br>• Full gameplay recording (no cuts)<br>• HUD stats: Score, Level, FPS counter<br>• Mobile touch demo (if applicable) | "Let's play the final version. Play the polished asteroids with all bugs fixed. Smooth 60fps, responsive controls, persistent high scores..<br><br>Notice: particle systems, asteroids-specific mechanics, and localStorage persistence — that's requestAnimationFrame with fixed timestep running at 60fps on a single Canvas context." | **Live HUD overlay**: `FPS: 60` `Entities: ~48 (entities + particles + UI)` |
| **4:00-4:20** | • Stats card: Lines of code, Dev time, Fix cycles, Final bug count<br>• Next episode teaser: Next game thumbnail + title | "Neon Asteroids - Day 11: 488 lines, 2.4 hours, 1 fix cycles, 0 bugs shipped.<br><br>Tomorrow: **Neon Tetris - Day 12**. Tetris with ghost piece, hold, next queue, SRS rotation." | **End card**: `Daily Game Studio Devlog #11`<br>`Next: #12 Neon Tetris - Day 12`<br>`Subscribe for daily AI gamedev` |
...
**Title**: `Daily Game Studio Devlog #11: Neon Asteroids - Day 11 — AI Builds & Fixes a Game in 24h`
...
🎮 PLAY Neon Asteroids - Day 11: https://dailygamestudio.github.io/daily-games/games/game-011/
📂 SOURCE CODE: https://github.com/dailygamestudio/daily-games/tree/main/games/game-011
🤖 HERMES AGENT: https://hermes-agent.nousresearch.com/

CHAPTERS:
0:00 Cold Open — Broken vs Fixed
0:15 The Challenge: Asteroids clone with ship inertia, screen wrap, splitting asteroids
...
**Thumbnail Text**: `AI Built This Game` + `Then Fixed Its Own Bugs` + `#11`
