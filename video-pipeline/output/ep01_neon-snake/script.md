...
|-----------|---------------------|----------------------|--------------------------------|
| **0:00-0:08** | **[COLD OPEN]**<br>• Split screen: Broken game (red console errors) → Fixed game (smooth 60fps)<br>• Overlay: "AI built this in 24 hours. Then it broke itself." | *(No voiceover — ambient synth music, glitch SFX on broken side)* | **Lower-third**: `Daily Game Studio Devlog #01`<br>`Neon Snake` |
| **0:08-0:15** | **[TITLE SEQUENCE]**<br>• Fast montage: Code generating → Playwright test running → NIM API fixing → Git push → Live on GitHub Pages<br>• Series logo animation | "Every day, an AI agent builds a complete HTML5 game from scratch. But the real story isn't the code it writes — it's the code it *fixes*." | **Animated title card**: `Daily Game Studio Devlog`<br>Episode `01: Neon Snake` |
| **0:15-0:35** | **[ACT 1: THE CHALLENGE]**<br>• Screen-recorded gameplay of Neon Snake (clean run)<br>• Diagram: Game genre classification + core mechanic<br>• Code snippet: Main game loop / entity system | "Day 1. The assignment: Classic Snake with neon aesthetics, particle effects, and progressive speed. Grid-based movement, collision detection (walls + self), food spawning, score persistence, speed scaling every 5 foods.<br><br>No frameworks. No libraries. Vanilla HTML5 Canvas, 400 lines max. The AI has one shot to get the physics, input, and rendering right." | **Code callout**: `gameLoop()` / `update()` / `draw()`<br>**Diagram**: `snake_mechanic.excalidraw` |
| **0:35-1:00** | • Archive: First commit diff (green lines only)<br>• index.json entry highlight<br>• GitHub Pages URL typing | "First commit pushed at 2026-08-13 00:00. Live at `dailygamestudio.github.io/daily-games/games/neon-snake/`<br><br>But the self-healing pipeline hadn't run yet. And that's where things get interesting." | **Overlay**: `git log --oneline -1`<br>`unknown Initial commit` |
...
| **2:00-2:30** | • Retry loop: Attempt 1 → still failing → Attempt 2 → PASS<br>• Test report: Green checkmarks | "First fix attempt: No fixes needed.<br>Second attempt: N/A.<br><br>The auto-fixer has a 3-retry limit with 30-second backoff. This game took 0 attempts and 0 minutes." | **Terminal log**:<br>`✅ neon-snake fixed and verified!`<br>`🔄 Cycle 1 complete` |
...
| **3:00-3:30** | • Version bump in index.json: `"version": 1` → `"version": 2`<br>• Git commit: "Auto-fix: Self-healing bug fixes"<br>• GitHub Actions deploy log | "Version bumped. Committed. Pushed. GitHub Pages rebuilds in ~2 minutes.<br><br>Now the live version is the *fixed* version. Players never see the broken one." | **Overlay**: `git diff index.json`<br>`+ "version": 2`<br>`+ "last_updated": "2026-08-17"` |
| **3:30-4:00** | **[ACT 4: PLAYTEST & REFLECTION]**<br>• Full gameplay recording (no cuts)<br>• HUD stats: Score, Level, FPS counter<br>• Mobile touch demo (if applicable) | "Let's play the final version. Play the polished snake with all bugs fixed. Smooth 60fps, responsive controls, persistent high scores..<br><br>Notice: particle systems, snake-specific mechanics, and localStorage persistence — that's requestAnimationFrame with fixed timestep running at 60fps on a single Canvas context." | **Live HUD overlay**: `FPS: 60` `Entities: ~32 (entities + particles + UI)` |
| **4:00-4:20** | • Stats card: Lines of code, Dev time, Fix cycles, Final bug count<br>• Next episode teaser: Next game thumbnail + title | "Neon Snake: 327 lines, 1.6 hours, 0 fix cycles, 0 bugs shipped.<br><br>Tomorrow: **Neon Breakout**. Brick-breaking with paddle physics, ball trails, and progressive levels." | **End card**: `Daily Game Studio Devlog #01`<br>`Next: #02 Neon Breakout`<br>`Subscribe for daily AI gamedev` |
...
🎮 PLAY Neon Snake: https://dailygamestudio.github.io/daily-games/games/neon-snake/
📂 SOURCE CODE: https://github.com/dailygamestudio/daily-games/tree/main/games/neon-snake
🤖 HERMES AGENT: https://hermes-agent.nousresearch.com/

CHAPTERS:
0:00 Cold Open — Broken vs Fixed
0:15 The Challenge: Classic Snake with neon aesthetics, particle effects, and progressive speed
...
**Thumbnail Text**: `AI Built This Game` + `Then Fixed Its Own Bugs` + `#01`
