...
|-----------|---------------------|----------------------|--------------------------------|
| **0:00-0:08** | **[COLD OPEN]**<br>• Split screen: Broken game (red console errors) → Fixed game (smooth 60fps)<br>• Overlay: "AI built this in 24 hours. Then it broke itself." | *(No voiceover — ambient synth music, glitch SFX on broken side)* | **Lower-third**: `Daily Game Studio Devlog #22`<br>`Neon Flow - Day 22` |
| **0:08-0:15** | **[TITLE SEQUENCE]**<br>• Fast montage: Code generating → Playwright test running → NIM API fixing → Git push → Live on GitHub Pages<br>• Series logo animation | "Every day, an AI agent builds a complete HTML5 game from scratch. But the real story isn't the code it writes — it's the code it *fixes*." | **Animated title card**: `Daily Game Studio Devlog`<br>Episode `22: Neon Flow - Day 22` |
| **0:15-0:35** | **[ACT 1: THE CHALLENGE]**<br>• Screen-recorded gameplay of Neon Flow - Day 22 (clean run)<br>• Diagram: Game genre classification + core mechanic<br>• Code snippet: Main game loop / entity system | "Day 22. The assignment: Pipe connection puzzle with flow direction, valves, pressure sensors. Directed graph flow, valve state toggles, pressure equalization, leak detection, flow rate balancing.<br><br>No frameworks. No libraries. Vanilla HTML5 Canvas, 400 lines max. The AI has one shot to get the physics, input, and rendering right." | **Code callout**: `gameLoop()` / `update()` / `draw()`<br>**Diagram**: `flow_mechanic.excalidraw` |
| **0:35-1:00** | • Archive: First commit diff (green lines only)<br>• index.json entry highlight<br>• GitHub Pages URL typing | "First commit pushed at 2026-08-19 00:00. Live at `dailygamestudio.github.io/daily-games/games/game-022/`<br><br>But the self-healing pipeline hadn't run yet. And that's where things get interesting." | **Overlay**: `git log --oneline -1`<br>`unknown Initial commit` |
...
| **2:00-2:30** | • Retry loop: Attempt 1 → still failing → Attempt 2 → PASS<br>• Test report: Green checkmarks | "First fix attempt: No fixes needed.<br>Second attempt: N/A.<br><br>The auto-fixer has a 3-retry limit with 30-second backoff. This game took 0 attempts and 0 minutes." | **Terminal log**:<br>`✅ game-022 fixed and verified!`<br>`🔄 Cycle 1 complete` |
...
| **3:30-4:00** | **[ACT 4: PLAYTEST & REFLECTION]**<br>• Full gameplay recording (no cuts)<br>• HUD stats: Score, Level, FPS counter<br>• Mobile touch demo (if applicable) | "Let's play the final version. Play the polished flow with all bugs fixed. Smooth 60fps, responsive controls, persistent high scores..<br><br>Notice: particle systems, flow-specific mechanics, and localStorage persistence — that's requestAnimationFrame with fixed timestep running at 60fps on a single Canvas context." | **Live HUD overlay**: `FPS: 60` `Entities: ~65 (entities + particles + UI)` |
| **4:00-4:20** | • Stats card: Lines of code, Dev time, Fix cycles, Final bug count<br>• Next episode teaser: Next game thumbnail + title | "Neon Flow - Day 22: 659 lines, 3.3 hours, 0 fix cycles, 0 bugs shipped.<br><br>Tomorrow: **Neon Conduit - Day 23**. Energy routing puzzle with transformers, switches, load balancing." | **End card**: `Daily Game Studio Devlog #22`<br>`Next: #23 Neon Conduit - Day 23`<br>`Subscribe for daily AI gamedev` |
...
**Title**: `Daily Game Studio Devlog #22: Neon Flow - Day 22 — AI Builds & Fixes a Game in 24h`
...
🎮 PLAY Neon Flow - Day 22: https://dailygamestudio.github.io/daily-games/games/game-022/
📂 SOURCE CODE: https://github.com/dailygamestudio/daily-games/tree/main/games/game-022
🤖 HERMES AGENT: https://hermes-agent.nousresearch.com/

CHAPTERS:
0:00 Cold Open — Broken vs Fixed
0:15 The Challenge: Pipe connection puzzle with flow direction, valves, pressure sensors
...
**Thumbnail Text**: `AI Built This Game` + `Then Fixed Its Own Bugs` + `#22`
