...
**Episode**: #{{EPISODE_NUM:02d}} — {{GAME_TITLE}}  
...
|-----------|---------------------|----------------------|--------------------------------|
| **0:00-0:08** | **[COLD OPEN]**<br>• Split screen: Broken game (red console errors) → Fixed game (smooth 60fps)<br>• Overlay: "AI built this in 24 hours. Then it broke itself." | *(No voiceover — ambient synth music, glitch SFX on broken side)* | **Lower-third**: `Daily Game Studio Devlog #{{EPISODE_NUM:02d}}`<br>`{{GAME_TITLE}}` |
| **0:08-0:15** | **[TITLE SEQUENCE]**<br>• Fast montage: Code generating → Playwright test running → NIM API fixing → Git push → Live on GitHub Pages<br>• Series logo animation | "Every day, an AI agent builds a complete HTML5 game from scratch. But the real story isn't the code it writes — it's the code it *fixes*." | **Animated title card**: `Daily Game Studio Devlog`<br>Episode `{{EPISODE_NUM:02d}}: {{GAME_TITLE}}` |
| **0:15-0:35** | **[ACT 1: THE CHALLENGE]**<br>• Screen-recorded gameplay of {{GAME_TITLE}} (clean run)<br>• Diagram: Game genre classification + core mechanic<br>• Code snippet: Main game loop / entity system | "Day {{DAY_NUM}}. The assignment: {{GENRE_DESCRIPTION}}. {{MECHANIC_DETAIL}}.<br><br>No frameworks. No libraries. Vanilla HTML5 Canvas, 400 lines max. The AI has one shot to get the physics, input, and rendering right." | **Code callout**: `gameLoop()` / `update()` / `draw()`<br>**Diagram**: `{{MECHANIC_DIAGRAM}}` |
| **0:35-1:00** | • Archive: First commit diff (green lines only)<br>• index.json entry highlight<br>• GitHub Pages URL typing | "First commit pushed at {{FIRST_COMMIT_TIME}}. Live at `dailygamestudio.github.io/daily-games/games/{{GAME_PATH}}/`<br><br>But the self-healing pipeline hadn't run yet. And that's where things get interesting." | **Overlay**: `git log --oneline -1`<br>`{{FIRST_COMMIT_HASH}} {{FIRST_COMMIT_MSG}}` |
| **1:00-1:30** | **[ACT 2: THE STRUGGLE]**<br>• Playwright test recording: Red failures, console errors<br>• Bug cards sliding in: `CRITICAL: JS_ERROR`, `MAJOR: MISSING_START_BUTTON`<br>• Terminal: `node test-runner.js --priority-only` | "The self-healing loop kicks in. Playwright loads the live page, simulates keystrokes, checks for game-over state.<br><br>And it finds: {{BUG_COUNT}} bugs. {{CRITICAL_COUNT}} critical. The game *looks* done. It's not." | **Bug cards** (animated):<br>🔴 `{{BUG_1_TYPE}}` — `{{BUG_1_MSG}}`<br>🟠 `{{BUG_2_TYPE}}` — `{{BUG_2_MSG}}`<br>... |
| **1:30-2:00** | • Split view: Buggy code (left) → NIM API prompt (center) → Fixed code (right)<br>• Highlight: Specific line causing crash<br>• Auto-fixer.js execution log | "Each bug gets packaged into a structured prompt for NIM API — Nemotron 3 Ultra. The prompt includes: the full HTML, the bug list with severity, and strict requirements: fix everything, keep the neon aesthetic, add pause/menu overlays, support touch.<br><br>Let's look at the worst offender: **{{WORST_BUG_TYPE}}**." | **Code diff** (syntax highlighted):<br>```diff<br>- {{BUGGY_LINE}}<br>+ {{FIXED_LINE}}<br>``` |
| **2:00-2:30** | • Retry loop: Attempt 1 → still failing → Attempt 2 → PASS<br>• Test report: Green checkmarks | "First fix attempt: {{ATTEMPT_1_RESULT}}.<br>Second attempt: {{ATTEMPT_2_RESULT}}.<br><br>The auto-fixer has a 3-retry limit with 30-second backoff. This game took {{TOTAL_ATTEMPTS}} attempts and {{TOTAL_FIX_TIME}} minutes." | **Terminal log**:<br>`✅ {{GAME_ID}} fixed and verified!`<br>`🔄 Cycle {{CYCLE_NUM}} complete` |
| **2:30-3:00** | **[ACT 3: THE BREAKTHROUGH]**<br>• Side-by-side: Before fix / After fix gameplay<br>• Code walkthrough: The architectural fix (not just patch)<br>• Diagram: Why this fix solves the root cause | "Here's what actually broke: {{ROOT_CAUSE_EXPLANATION}}.<br><br>The fix wasn't just 'add missing button' — it was **{{ARCHITECTURAL_INSIGHT}}**.<br><br>This is the pattern across all 54 games: the AI learns to build *testable* architecture, not just working code." | **Architecture diagram**: `{{ARCH_DIAGRAM}}`<br>**Key code block**: `{{KEY_FIX_FUNCTION}}` |
| **3:00-3:30** | • Version bump in index.json: `"version": 1` → `"version": 2`<br>• Git commit: "Auto-fix: Self-healing bug fixes"<br>• GitHub Actions deploy log | "Version bumped. Committed. Pushed. GitHub Pages rebuilds in ~2 minutes.<br><br>Now the live version is the *fixed* version. Players never see the broken one." | **Overlay**: `git diff index.json`<br>`+ "version": 2`<br>`+ "last_updated": "{{FIX_DATE}}"` |
| **3:30-4:00** | **[ACT 4: PLAYTEST & REFLECTION]**<br>• Full gameplay recording (no cuts)<br>• HUD stats: Score, Level, FPS counter<br>• Mobile touch demo (if applicable) | "Let's play the final version. {{GAMEPLAY_DESCRIPTION}}.<br><br>Notice: {{TECHNICAL_OBSERVATION}} — that's {{TECH_DETAIL}} running at 60fps on a single Canvas context." | **Live HUD overlay**: `FPS: {{FPS}}` `Entities: {{ENTITY_COUNT}}` |
| **4:00-4:20** | • Stats card: Lines of code, Dev time, Fix cycles, Final bug count<br>• Next episode teaser: Next game thumbnail + title | "{{GAME_TITLE}}: {{LOC}} lines, {{DEV_TIME}} hours, {{FIX_CYCLES}} fix cycles, 0 bugs shipped.<br><br>Tomorrow: **{{NEXT_GAME_TITLE}}**. {{NEXT_GAME_TEASER}}." | **End card**: `Daily Game Studio Devlog #{{EPISODE_NUM:02d}}`<br>`Next: #{{NEXT_EPISODE_NUM:02d}} {{NEXT_GAME_TITLE}}`<br>`Subscribe for daily AI gamedev` |
...
**Title**: `Daily Game Studio Devlog #{{EPISODE_NUM:02d}}: {{GAME_TITLE}} — AI Builds & Fixes a Game in 24h`
...
🎮 PLAY {{GAME_TITLE}}: https://dailygamestudio.github.io/daily-games/games/{{GAME_PATH}}/
📂 SOURCE CODE: https://github.com/dailygamestudio/daily-games/tree/main/games/{{GAME_PATH}}
🤖 HERMES AGENT: https://hermes-agent.nousresearch.com/

CHAPTERS:
0:00 Cold Open — Broken vs Fixed
0:15 The Challenge: {{GENRE_DESCRIPTION}}
1:00 The Struggle: {{BUG_COUNT}} Bugs Found
2:30 The Breakthrough: {{ARCHITECTURAL_INSIGHT}}
...
**Thumbnail Text**: `AI Built This Game` + `Then Fixed Its Own Bugs` + `#{{EPISODE_NUM:02d}}`
